...
mod_f0 <- lm(formula = f0 ~ island, data = tidy_f0)
```
...
f0_summary <- tidy(mod_f0)

knitr::kable(f0_summary)
...
- **So, it seems that Mauke has a higher rate of voiceless [w], when [w] is realized**

- the other 3 islands typically have f0 \> 100 Hz, so these tokens are voiced

- **SO,** we also expect f2 values to also say that Mauke has more [w] than [v] surfacing
...
- Atiu on average has more [w]-surfacing and Rarotonga has the least occurrences of [w]-surfacing (more [v])
