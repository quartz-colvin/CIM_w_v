...
The duration data were analyzed using a linear mixed effects model. Duration was the criterion with \textit{island, speaker} and \textit{word} as predictors. Main effects and the interactions between \textit{island, speaker} and \textit{word} were assessed using nested model comparisons. Experiment-wise, alpha was set at 0.05.
...
\textcolor{red}{
\begin{itemize}
\item FIXED EFFECTS: coefficients (betas), standard errors (SE), t-values, p-values, effect size
\item RANDOM EFFECTS: variance components (std dev), residual variance
\item MODEL FIT \& ASSUMPTIONS: BIC/AIC (lower values indicate better model fit when comparing models), residual plots (qq-plot, residual vs fitted, residual vs order)
\end{itemize}
}
