...
\hline
  & betas & SE & t-val & df \\
\hline
(intercept) & 0.064761 & 0.013130 & 4.932 & 6.339137 \\
...
% standard deviation is a measure of how dispersed the data is in relation to the mean. small standard deviation means more data is clustered around the mean;  larger means its more spread out 

% variance is the average squared distance between the data points and the mean. higher values indicate greater variability

\textcolor{red}{interpret random effects ??? }
...
\subsubsection{Model fit and assumptions:}

\textcolor{red}{interpret ??? }

\begin{itemize}
\item BIC/AIC (lower values indicate better model fit when complaring models) = ???
\item residual plots (qq-plot, residual vs. fitted, residual vs order) = ???
\end{itemize}




Significance of main effects and all possible interactions were assessed using hierarchical partitioning of the variance via nested model comparisons. Experiment-wise, alpha was set at 0.05.
...
The \textbf{standard error (SE)} for each island were {large, around 11-15, indicating that the mean of the sample is not an accurate reflection of the actual populations mean.  The \textbf{t-values} for each island were all lower than 2, indicating that they are not statistically significant between the islands. 
...
\hline
  & betas & SE & t-val  & df \\
\hline
intercept & 57.919 & 10.864 & 5.331  & 6.184 \\
...
The \textbf{degrees of freedom (df)} for all islands were around 6. This indicates \textcolor{red}{what the hell's it indicate???}. For Mauke, beta was 2.534, which indicates \textcolor{red}{???what!}. For Penrhyn, beta was -7.853, which indicates \textcolor{red}{???what!}. For Rarotonga, beta was 2.095, which indicates \textcolor{red}{???what!}. 
...
\textcolor{red}{interpret fixed effects ??? }
...
\hline
  & betas & SE & t-val & df \\
\hline
intercept & 140.935 & 24.584 & 5.733 & 3.466 \\
...
\hline
  & betas & SE & t-val & df \\
\hline
intercept & 1142.411 & 83.412 & 13.696 & 5.643 \\
...
\pagebreak
\subsection{P-values and formant models}

\textcolor{red}{WHAT TO DO WITH THIS SECTION??? }
